...
class MundoDosCiliosApp(MDApp):
    def build(self):
        manipulador = ManipulaJanela(500, 700)
        ...
        background = Image(source="images/mundodoscilios.png", allow_stretch=True, keep_ratio=False)
        layout.add_widget(background)

        box_layout = MDBoxLayout(orientation="vertical", padding=10, spacing=5)
        servicos = [
            ("Agendar", "images/agenda.png"),
            ("Minha Conta", "images/minhaconta.png"),
            ("Sobre Nós", "images/sobrenos.png"),
            ("Nossa Equipe", "images/nossaequipe.png"),
            ("Efeito Fox", "images/efeitofox.png"),
            ("Efeito Harmony","images/efeitoharmony.png"),
            ("Técnica Hibrido","images/tecnicahibrido.png"),
            ("Volume Power","images/volumepower.png"),
            ("Volume Light","images/volumelight.png"),
            ("Volume Brasileiro","images/volumebrasileiro.png"),
            ("Volume Russo","images/volumerusso.png"),
            ("Volume Clássico","images/volumeclassico.png"),
            ("SOS Capilar","images/soscapilar.png"),
        ...
        sm.add_widget(Agendar(name="Agendar"))
...
class Agendar(Screen):
        def __init__(self, **kwargs):
            super().__init__(**kwargs)
            self.layout = MDBoxLayout(orientation="vertical", padding=20)

            self.botao_calendario = MDRaisedButton(text="Calendário", pos_hint={"center_x": .5, "center_y": .5}, on_release=self.calendario)
            self.layout.add_widget(self.botao_calendario)

            self.layout.add_widget(BotaoVoltar())
            self.add_widget(self.layout)

        def botao_salvar(self, instance, value, date_range):
            label = MDLabel(text=f"Agendamento concluído para {value}")
            self.layout.add_widget(label)

        def botao_cancelar(self, instance, value):
            label = MDLabel(text=f"Agendamento cancelado.")
            self.layout.add_widget(label)

        def calendario(self, *args):
            date_dialog = MDDatePicker()
            date_dialog.bind(on_save=self.botao_salvar, on_cancel=self.botao_cancelar) 
            date_dialog.open()
